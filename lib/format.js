 ...
 * @returns e.g. `"¥12.34"`, `"$5.00"`, `"12.34 CHF"`, `"¥0"`, `"—"`.
 */
export function formatMoney(value, currency = 'CNY') {
  ...
  // An exact zero is a zero, not a six-decimal hole: the small-amount precision
  // below exists to keep a sub-cent FIGURE readable, and applying it to `0`
  // rendered a spent-out account as `¥0.000000` next to a `¥137.00` cap.
  if (value === 0) return symbol === undefined ? `0 ${code}` : `${symbol}0`
 ...
 *
 * @returns e.g. `"12.34"`, `"0.0539"`, `"0.000123"`, `"0"`, `"—"`.
 */
