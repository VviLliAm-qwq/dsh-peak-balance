...
export function createTracker() {
  ...
  return {
    ...
    turnCost(atMs = Date.now()) {
      return estimateCostCny(state.turn, state.model, atMs)
    },

    /** Estimated cost of the whole session, or `undefined`. */
    sessionCost(atMs = Date.now()) {
      return estimateCostCny(state.session, state.model, atMs)
    },
     ...
     */
    endTurn(atMs = Date.now()) {
      const tokens = this.turnTokens()
      const cost = this.turnCost(atMs)
      state.turnIndex += 1
      if (tokens <= 0) {
