...
import { emptyBuckets, estimateCostCny, totalTokens } from './pricing.js'
...
function freshState() {
  return {
    model: '',
    /**
     * Provider route the focused conversation is spending through.
     *
     * Read off `request/header.config.provider`, which the host fills for every
     * route (official, pi-ai catalog, a hand-declared gateway). It decides which
     * account the quota section asks about, so it is tracked next to the model.
     */
    provider: '',
     ...
     */
    turnModel: '',
    /** Provider of the first report in the running turn (see {@link turnModel}). */
    turnProvider: '',
    turnAt: undefined,
    turnIndex: 0,
    lastTurn: undefined,
...
export function createTracker() {
  ...
  return {
    ...
    /** Provider route of the focused conversation; the quota target in `auto`. */
    get provider() {
      return state.provider
    },

    /** Set the provider seen on `request/header` (quota-target key). */
    ...
    onUsage(usage, atMs = Date.now()) {
      ...
      if (state.turnProvider === '') state.turnProvider = state.provider
    ...
    beginTurn() {
      ...
      state.turnModel = ''
      // The provider belongs to the reset too: a turn that never emitted
      // `turn/end` would otherwise hand its route to the next round's settled
      // figure, while `turnModel`/`turnAt` were already being cleared here.
      state.turnProvider = ''
    ...
    endTurn(atMs = Date.now(), customRates = undefined) {
      ...
      const provider = state.turnProvider !== '' ? state.turnProvider : state.provider
      state.turnIndex += 1
      state.turn = emptyBuckets()
      state.turnModel = ''
      state.turnProvider = ''
      ...
      const settled = { cost, tokens, model, provider, turn: state.turnIndex }
    ...
    snapshot() {
      return {
        ...
        turnProvider: state.turnProvider,
        turnAt: state.turnAt,
    ...
    reset() {
      ...
      state.turnProvider = fresh.turnProvider
