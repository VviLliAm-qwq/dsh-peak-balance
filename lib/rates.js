...
/**
 * Split a user-typed rate target into its provider and model halves.
 *
 * The syntax is `provider:model`, and the split is on the FIRST colon: provider
 * route ids never contain one, while model ids sometimes do
 * (`inclusionai/ling-3.0-flash-sante:free`).
 *
 * @param token - e.g. `commandcode:deepseek/deepseek-v4.1-flash`, or `deepseek-flash`.
 * @returns `{ provider, model }`; `provider` is `''` for a bare model id.
 */
export function parseRateTarget(token) {
  const text = typeof token === 'string' ? token.trim() : ''
  if (text === '') return { provider: '', model: '' }
  const colon = text.indexOf(':')
  if (colon <= 0 || colon === text.length - 1) return { provider: '', model: text }
  return { provider: text.slice(0, colon), model: text.slice(colon + 1) }
}

/** Stored key for one provider/model pair (`provider:base`, or the bare base id). */
export function ratesKeyOf(provider, model) {
 ...
 *
 * A stored key is either a bare base model id (everything written before v2) or
 * a `provider:base` pair. Both are normalized through {@link ratesKeyOf}, so a
 * hand-edited `commandcode:deepseek/deepseek-v4.1-flash` and a typed
 * `commandcode:deepseek-v4.1-flash` land on the same entry.
 */
export function parseRates(text, now = 0) {
  ...
  for (const [model, raw] of Object.entries(source)) {
    const target = parseRateTarget(model)
    const key = ratesKeyOf(target.provider, target.model)
    if (key === '') continue
