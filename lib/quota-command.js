...
/**
 * An epoch instant as ISO text, or `undefined` when it is out of range.
 *
 * `new Date(1e16).toISOString()` throws a RangeError, and a reset instant that
 * large can only come from a provider spec — but a diagnostic command must
 * degrade to "unprintable" rather than fail its whole report for one field.
 */
...
function meterLine(meter) {
  ...
  if (typeof meter.resetAt === 'number') {
    const reset = isoInstant(meter.resetAt)
...
export function renderQuotaReport(lang, report) {
  ...
  if (typeof report?.updatedAt === 'number') {
    const at = isoInstant(report.updatedAt)
