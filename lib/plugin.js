...
import { readHostModelProvider } from './model-pref.js'
    ...
    fields: [
      ...
      },
      {
        ...
        options: [
          ...
          { value: 'lifetimeSpend', label: 'Lifetime spend', descriptions: { zh: '累计已用', en: 'Lifetime spend' } },
...
function appendLogLine(path, line) {
  try {
    if (statSync(path).size > MAX_LOG_BYTES) {
      // Byte-accurate, unlike a string `slice`: MAX_LOG_BYTES is a byte budget,
      // and slicing the decoded text by the same number would keep up to three
      // times as many bytes for a non-ASCII log — and could cut a multi-byte
      // character in half. The cut starts at a line boundary so the trimmed log
      // never begins with half a record.
      const tail = readFileSync(path).subarray(-Math.floor(MAX_LOG_BYTES / 2))
      const newline = tail.indexOf(0x0a)
      writeFileSync(path, newline === -1 ? tail : tail.subarray(newline + 1))
    }
  } catch {
...
export function apply(ctx, config) {
  ...
  try {
    ...
    /** The refresh currently in flight, or `undefined` (one request at a time). */
    let quotaInFlight
    /** Rotation cursor for the `rotate` metric (advanced by the phase timer). */
    let quotaTick = 0
    /**
     * Settled measured spend by conversation id (bounded, recency-ordered).
     *
     * One measurement per CONVERSATION, not one per plugin: the provider's
     * counter is global, but the turn it belongs to is not — so the baseline
     * lives on each tracker entry (see `beginTurnSpend`). This map exists only
     * to serve a conversation the host disposed and resumed.
     */
    ...
    /** Command names whose localized completion tree this activation registered. */
    const commandTreeRoots = new Set()
      ...
      historyScan = (async () => {
        try {
          const result = await scanSessions({
            ...
            // A scan can finish short of the whole corpus (a log whose tail frame
            // is still being written, or more logs than the cache cap) — that is
            // worth a line in the plugin's own log rather than a silent
            // difference between the board and the session directory.
            onWarning: message => log.warn(`history scan: ${message}`),
            /**
             * Paint what the scan has reduced so far.
             *
             * A cold cache has nothing to show until the scan ends, and the scan
             * walks the newest logs first — so publishing each batch puts the
             * recent weeks on screen within the first ticks instead of leaving a
             * loading state for seconds. The status stays whatever the scan set
             * (`loading` for a cold board, `ready` for a refresh behind an
             * on-screen grid), so the progress figure keeps ticking either way.
             * The ticks are already coalesced: one repaint per
             * `PROGRESS_MIN_INTERVAL_MS` at most.
             */
            onRecords: (records, progress) => {
              ...
              historyRecords = records
              mergeHistory({ progress, refreshing: true, error: undefined })
              publishHistory()
            },
          })
          ...
          log.info(
            `history scan files=${result.stats.files} scanned=${result.stats.scanned} reused=${result.stats.reused} ` +
              `resumed=${result.stats.resumedFiles} events=${result.stats.events} skipped=${result.stats.skippedEvents} ` +
              `ms=${result.stats.durationMs} truncated=${result.stats.truncatedLogs} dropped=${result.stats.droppedFiles}` +
              (result.stats.migratedFrom === undefined ? '' : ` migratedFrom=${result.stats.migratedFrom}`),
          )
          return result
        } catch (error) {
          if (!disposed) {
            mergeHistory({
              // A scan that painted part of the board already has something
              // worth keeping on screen; only an empty one falls back to error.
              status: haveRecords || historyRecords.length > 0 ? 'ready' : 'error',
    ...
    const quotaCommandActions = {
      status: async () => {
        const target = resolveTarget({ provider: observedProvider(), preference: preferredProvider(), config: resolved })
      ...
      check: async provider => {
        ...
        const target = requested === ''
          ? resolveTarget({ provider: '', preference: preferredProvider(), config: resolved })
    ...
    const registerCommandTrees = () => {
      // Every contribution gets a tree, `/quota` included: the registry
      // descriptor carries no translations, so without one that command's menu
      // entry falls back to its English registry description. Tracking the roots
      // by name (rather than by disposer count) is what lets the retry loop fill
      // in a root that failed while the others already landed.
      const definitions = [
        ...createCommandDefinitions({ actions: commandActions, getLang: () => lang }),
        createQuotaCommand({ actions: quotaCommandActions, getLang: () => lang }),
      ]
      const missing = definitions.filter(definition => !commandTreeRoots.has(definition.name))
      ...
      for (const trees of seamServices(ctx, 'tuiCommandTrees')) {
        for (const definition of missing) {
          try {
            ...
            if (typeof dispose === 'function') {
              commandTreeDisposers.push(dispose)
              commandTreeRoots.add(definition.name)
            }
          } catch (error) {
            warnOnce(log, seenWarnings, `command tree ${definition.name} failed: ${messageOf(error)}`)
          }
        }
        if (commandTreeRoots.size > 0) break
      }
      if (commandTreeRoots.size > 0) {
        log.info(`command tree registered roots=${commandTreeRoots.size}`)
      }
      return commandTreeRoots.size === definitions.length
    }
    ...
    /** Remember one conversation's measured spend (bounded, recency-ordered). */
    const rememberSpend = (id, spend) => {
      if (id === '') return
      spendById.delete(id)
      spendById.set(id, spend)
      while (spendById.size > MAX_SETTLED) {
        const oldest = spendById.keys().next().value
        if (oldest === undefined) break
        spendById.delete(oldest)
      }
    }
     ...
     * @returns `{ id, tracker, turnSpend, spendRetries, spendRetryTimer, steps,
     *   turnOpen }`; recency is refreshed so eviction drops the least recently
     *   active session, never the focused one. `steps` maps `turn:step` to the
     *   instant that request STARTED (the tier a request is billed in is the one
     *   it ran in, and the assistant message lands after the answer); `turnOpen`
     *   marks a round as accepting subagent spend.
     */
    const trackerEntry = session => {
      ...
      const entry = {
        ...
        // The measured-spend baseline belongs to THIS conversation: one shared
        // instance let a parked conversation's `turn/start` overwrite the
        // baseline the focused conversation was about to settle against.
        turnSpend: createTurnSpend(),
        spendRetries: 0,
    ...
    /**
     * The provider the host's persisted `/model` route names, or `''`.
     *
     * The stand-in for a conversation restored at boot: the host replays its
     * history privately (no `session/event`), so until that conversation's first
     * request nothing has named its route — and the historical default would
     * report a different account than the one the user is about to spend from.
     * The `/model` picker persists its choice for exactly this restart window,
     * so it is the honest answer while the conversation stays silent.
     */
    const preferredProvider = () => readHostModelProvider()
    ...
    const refreshQuota = async (options = {}) => {
      if (disposed || resolved.showBalance !== true) return undefined
      // One request at a time. A plain call joins the one already in flight —
      // its answer is fresh enough for a repaint — while a FORCED call (the one
      // `turn/end` settles a measured spend against) waits for it and then reads
      // again: reusing the in-flight answer would settle the turn against a
      // snapshot taken before the provider accounted for it, which reads as
      // "this turn was free" until the retry budget runs out.
      while (quotaInFlight !== undefined) {
        if (options.force !== true) return quotaInFlight
        await quotaInFlight
        if (disposed) return undefined
      }
      const now = Date.now()
      if (options.force !== true && now - quotaRequestedAt < MIN_BALANCE_GAP_MS) return undefined
      ...
      try {
        ...
        // Guarded so a pass that was superseded never clears its successor.
        if (quotaInFlight === pass) quotaInFlight = undefined
      }
    }

    /** Drop one conversation's pending settlement retry. */
    const clearSpendRetry = entry => {
      if (entry?.spendRetryTimer === undefined) return
      clearTimeout(entry.spendRetryTimer)
      entry.spendRetryTimer = undefined
    }

    /**
     * Capture the provider's spend counter as one conversation's turn begins.
     *
     * The baseline comes from the last snapshot in hand (at most one refresh
     * interval old) — the same reading the settlement compares against, so a
     * provider that accounts a request late is handled by the retry below rather
     * than by a second clock.
     *
     * @param entry - the tracker entry whose turn is starting.
     */
    const beginTurnSpend = entry => {
      entry.spendRetries = 0
      clearSpendRetry(entry)
      if (resolved.turnSpendMode === 'estimate') {
        entry.turnSpend.reset()
        return
      }
      entry.turnSpend.begin(quotaSnapshot?.spendCounter)
    }

    /**
     * Settle the measured spend of one conversation's turn that just ended.
     *
     ...
     *
     * @param entry - the tracker entry whose turn ended. The measurement is
     *   attributed to THIS conversation, never to whichever one happens to be
     *   focused by the time the provider answers.
     * @param options.force - settle even when the counter has not moved.
     */
    const settleTurnSpend = (entry, options = {}) => {
      if (resolved.turnSpendMode === 'estimate') return
      if (entry === undefined) return
      const result = entry.turnSpend.settle(quotaSnapshot?.spendCounter, { force: options.force === true })
      if (result.state === 'settled') {
        const spend = { spend: { value: result.spent, unit: result.unit }, spendExact: result.exact }
        // Two homes, on purpose: the entry serves the focused line directly, and
        // the id-keyed map serves a conversation the host disposed and resumed.
        entry.spend = spend
        rememberSpend(entry.id, spend)
        publish()
        return
      }
      if (result.state === 'pending') scheduleSpendRetry(entry)
    }
     ...
     */
    const scheduleSpendRetry = entry => {
      if (disposed || entry === undefined || entry.spendRetryTimer !== undefined) return
      if (entry.spendRetries >= SPEND_RETRY_LIMIT) {
        entry.spendRetries = 0
        void refreshQuota({ force: true }).then(() => settleTurnSpend(entry, { force: true }))
        return
      }
      entry.spendRetries += 1
      entry.spendRetryTimer = setTimeout(() => {
        entry.spendRetryTimer = undefined
        void refreshQuota({ force: true }).then(() => settleTurnSpend(entry))
      }, SPEND_RETRY_MS)
      if (typeof entry.spendRetryTimer.unref === 'function') entry.spendRetryTimer.unref()
    }
    ...
    ctx.effect(function* lifecycle() {
          ...
          if (scope === undefined) {
            ...
            if (registered) {
              ...
              watchDisposer = scope.watch(next => {
                // Merge over the CURRENT values rather than over the defaults:
                // sanitizeConfig always returns the whole shape, so a host that
                // hands back only the changed keys would otherwise reset every
                // other setting to its default on each write.
                Object.assign(resolved, sanitizeConfig({ ...resolved, ...(next ?? {}) }))
      ...
      yield () => {
        disposed = true
        for (const timer of [retryTimer, phaseTimer, quotaTimer, rotateTimer, focusTimer, langTimer]) {
          if (timer !== undefined) clearInterval(timer)
        }
        // The spend retry is a per-conversation timeout rather than an interval:
        // every tracked entry owns one, so each is cleared on its own.
        for (const entry of trackers.values()) clearSpendRetry(entry)
        stopHistoryRefresh()
        retryTimer = undefined
