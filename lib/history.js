 ...
 * `./history-scan.js`): `{ id, subagent, models: { <bucket>: { days: { <day>:
 * { peak: counts, idle: counts } } } } }`, where `<bucket>` is the
 * `provider/model` key of {@link bucketKeyOf}. Usage is bucketed by the event's
 * own timestamp, so a day that straddles a peak boundary is priced correctly,
 * and every cost is a CNY estimate from the rate card in `./pricing.js`.
 *
 ...
 *
 * The input-side figures a provider reports are disjoint — the miss, the cache
 * hit and the cache write are three separate counts, and the provider's own
 * `totalTokens` agrees with their sum plus the output — so the denominator is
 * the whole prompt and no reported token is counted twice.
 *
...
export function levelOf(value, thresholds) {
  ...
  const [t1, t2, t3] = thresholds ?? [0, 0, 0, 0]
...
/**
 * Leading marker of a bucket key whose provider is unknown (`/model`).
 *
 * A provider-less model id may itself contain a slash
 * (`inclusionai/ling-3.0-flash-sante:free`); without the marker a later split at
 * the first `/` would read `inclusionai` as the provider and invent an account.
 */
export const BARE_KEY_PREFIX = '/'

/**
 * Bucket key of one (provider, model) pair.
 *
 * Buckets are keyed by `provider/model` rather than by model alone, because the
 * same weights bought through a subscription and through a pay-as-you-go key are
 * two different accounts, and the board attributes spend per account. A request
 * whose provider the host never named (every pre-0.4.0 record looks like that)
 * keeps the bare model id — marked with {@link BARE_KEY_PREFIX} when the id
 * itself contains a slash, so the split stays unambiguous.
 */
export function bucketKeyOf(provider, model) {
  ...
  return id.includes('/') ? `${BARE_KEY_PREFIX}${id}` : id
}

/** Split a bucket key back into `{ provider, model }` (inverse of the above). */
export function splitBucketKey(key) {
  const text = typeof key === 'string' ? key : ''
  if (text.startsWith(BARE_KEY_PREFIX)) return { provider: '', model: text.slice(BARE_KEY_PREFIX.length) }
...
/** A session record with nothing folded into it yet. */
export function emptyRecord() {
  return { id: '', subagent: false, seeded: false, createdAt: 0, lastTime: 0, models: {}, events: 0, skipped: 0 }
}

/**
 * A session fold that can be continued where an earlier one stopped.
 *
 * `foldSessionEvents` reduces a whole log in one pass, which is all the scan
 * needed while it read every byte of every log. A log that is still being
 * written — every `/hist` refresh touches the session the board is drawn in —
 * only gains frames at its *end*, though, and re-reading a whole file for a few
 * new events is what made those refreshes cost hundreds of milliseconds. The
 * fold is therefore a state rather than a function: push the entire event list
 * once (what `foldSessionEvents` does), or push a cached prefix's events first
 * and then only what was appended.
 *
 * Continuing is sound because a record only ever accumulates: buckets gain
 * counts and nothing is ever subtracted. The two pieces of state that are *not*
 * a pure accumulation are carried explicitly — the model/provider named by the
 * newest `request/header`, and the fork-seed cut, which is settled entirely
 * inside the prefix (the scan refuses to continue a fold whose cut never
 * resolved).
 *
 * A fresh fold therefore expects the whole log in one `push` — that is what
 * makes its header and fork-cut resolution identical to the single-pass
 * version. A continued fold must *not* re-resolve them from its batch: the
 * batch starts after the prefix, so an index into it would cut in the wrong
 * place.
 *
 * @param carry - a {@link SessionFold.carry} from an earlier fold, optionally
 *   alongside that fold's record (`{ ...carry, record }`). Omit it to start a
 *   fresh fold.
 * @returns `{ push, record, carry }`; `carry` is what the next fold needs and
 *   deliberately excludes the record, which the scan stores beside it.
 */
export function createSessionFold(carry = undefined) {
  // A carry that is not record-shaped is not a prefix this fold can continue:
  // starting fresh is the only reading that cannot mix two shapes.
  const carried = carry?.record
  const resumable = carried !== null && typeof carried === 'object' && !Array.isArray(carried)
  const record = resumable ? carried : emptyRecord()
  if (record.models === null || typeof record.models !== 'object' || Array.isArray(record.models)) {
    record.models = {}
  }
  let model = typeof carry?.model === 'string' ? carry.model : ''
  let provider = typeof carry?.provider === 'string' ? carry.provider : ''
  let headerSeen = carry?.headerSeen === true
  let seedLength = typeof carry?.seedLength === 'number' && Number.isFinite(carry.seedLength) && carry.seedLength > 0
    ? Math.floor(carry.seedLength)
    : 0
  let seedUnresolved = carry?.seedUnresolved === true
  let started = resumable

  /** Fold one batch of parsed events, in log order, into the record. */
  const push = events => {
    ...
    if (!started) {
      ...
      if (header !== undefined) {
        headerSeen = true
        record.id = typeof header.id === 'string' ? header.id : ''
        record.subagent = header.origin === 'subagent'
          || (typeof header.delegationDepth === 'number' && header.delegationDepth > 0)
        const rawSeedLength = header.seedLength
        seedLength = typeof rawSeedLength === 'number' && Number.isFinite(rawSeedLength) && rawSeedLength > 0
          ? Math.floor(rawSeedLength)
          : 0
        record.seeded = seedLength > 0 || header.isSeeded === true
        record.createdAt = typeof header.createdAt === 'number' ? header.createdAt : 0
        record.lastTime = record.createdAt
        // A header that only marks `isSeeded` falls back to the first
        // `session/end-seed` marker's position. No marker at all means no cut
        // was applied — and a fold continued past that point could not tell
        // whether a later batch carries one, so it reports the cut unresolved.
        if (seedLength === 0 && header.isSeeded === true) {
          seedCutIndex = list.findIndex(event => event?.type === 'session/end-seed')
          seedUnresolved = seedCutIndex === -1
        }
      }
    }
    for (let index = 0; index < list.length; index += 1) {
      ...
      const atMs = typeof event.time === 'number' ? event.time : record.createdAt
      counts.events = 1
  ...
  return {
    ...
    get carry() {
      return { model, provider, headerSeen, seedLength, seedUnresolved }
    },
  }
}
 ...
 * must not be counted again. The header's `seedLength` is the exact cut: it
 * equals the `seq` of the last `session/end-seed` event (a multi-generation fork
 * carries one marker per generation, and the child's own work starts after the
 * newest one). A header that only marks `isSeeded` falls back to the first
 * marker's position.
 *
 * @param events - parsed log events, in seq order (the *whole* log: the fold
 *   resolves the header and the fork cut from this one list).
...
export function buildHistoryView(records, options = {}) {
  ...
  /** Per-provider subtotals: which account the spend came from. */
  ...
  // Every cost on the board is a CNY estimate from the same rate card, so a
  // selected set of providers is always summable and the cost metric needs no
  // unit fallback (a model the card cannot price reports `costIncomplete`).
  ...
  const gridDays = keys.map((key, index) => {
    ...
    return {
      ...
      value: day === undefined ? 0 : metricValueOf(day, metric),
