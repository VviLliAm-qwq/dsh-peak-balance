...
import { bucketCostCny, rateSourceOf, resolveRateCard } from './pricing.js'
...
export function foldSessionEvents(events) {
  ...
  for (let index = 0; index < list.length; index += 1) {
    ...
    counts.events = 1
    const tiers = tierBucket(modelBucket(record, model === '' ? UNKNOWN_MODEL : model), dayKeyOf(atMs))
...
export function buildHistoryView(records, options = {}) {
  ...
  for (const record of records ?? []) {
    ...
    for (const [model, entry] of Object.entries(record.models ?? {})) {
      const source = rateSourceOf(model, customRates)
      const card = resolveRateCard(model, now, customRates)
      let modelEntry = models.get(model)
      ...
      for (const [dayKey, tiers] of Object.entries(entry?.days ?? {})) {
        ...
        addCounts(dayModelCounts(day, model), combined)
  ...
  const modelRows = [...models.values()]
    .sort((a, b) => totalTokensOf(b.counts) - totalTokensOf(a.counts) || a.model.localeCompare(b.model))
    .map(entry => ({
      model: entry.model,
      source: entry.source,
      counts: entry.counts,
