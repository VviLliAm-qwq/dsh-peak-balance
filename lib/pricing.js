 ...
 *
 * There is deliberately no per-provider unit table. Every figure this module
 * produces is a CNY estimate from the rate card above, so the board has exactly
 * one unit and never has to decide whether two rows may be added together; a
 * model the card cannot price is reported as unpriced instead.
 */
