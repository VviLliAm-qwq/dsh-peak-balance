 ...
 * cursor sits inside the input box's first column), so the line above it looks
 * aligned only when it starts on the same column instead of hugging the
 * terminal edge.
 */
export const VIEW_INDENT_CELLS = 1
...
export function createStatusView(store) {
  return function PeakBalanceStatusView({ React, ui }) {
    ...
    if (!warn) {
      return React.createElement(
        Box,
        { flexDirection: 'row', paddingLeft: VIEW_INDENT_CELLS },
