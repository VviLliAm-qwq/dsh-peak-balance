...
import { formatCny, formatPercent, formatTokens, formatTokensFull } from './format.js'
...
export function createHistoryScene(options) {
  ...
  return function TokenHistoryScene({ React, ui, close }) {
    ...
    // The cursor is state, but it can name a day the CURRENT view no longer
    // draws: changing the span (or a scan that rolls the window over) can drop
    // its column, and a key outside the grid makes `stepGrid` a no-op for every
    // arrow key. An unresolvable cursor therefore falls back to the default day.
    const cursorKey = view !== undefined && cursor !== undefined && gridPositionOf(view, cursor) !== undefined
      ? cursor
      : undefined
    const selectedKey = cursorKey ?? defaultKey
    ...
    const defaultStart = Math.max(0, totalWeeks - visibleWeeks)
    const requestedStart = Number.isFinite(windowStart) ? windowStart : defaultStart
    // `windowStart` is scroll state, but it must never hide the caret: the
    // window is re-clamped around the selected column every render, so a span
    // change or a fallback cursor cannot leave the selection off-screen.
    const start = totalWeeks === 0
      ? 0
      : windowStartFor(selectedPosition?.column ?? requestedStart, requestedStart, visibleWeeks, totalWeeks)
    ...
    // Every section charges the rows it will really draw, top margin included:
    // the scene owns a full-screen surface, so an unbudgeted row does not clip
    // politely — it pushes the frame around. `view` may still be undefined while
    // the first scan runs, and the compact branch below draws from that state.
    const perProvider = Array.isArray(view?.providers) ? view.providers : []
    const totalsRowCount = 1 /* sum */ + (perProvider.length > 1 ? 1 : 0) + 1 /* subagents */ + (view?.totals?.costIncomplete === true ? 1 : 0)
    const SECTION_MARGIN_TOP = 1
    ...
    // `view` is absent until the first publish, and the compact branch below
    // draws that state: nothing above it may dereference the view directly.
    ...
    // The detail card is the one section whose row count is a choice: its extra
    // field rows are spent only from what is left after the totals (the headline
    // figure) and the hint have been paid for, so a short terminal drops hover
    // fields before it drops the sum.
    const detailRoom = left - (1 + (framed ? 2 : 0) + SECTION_MARGIN_TOP) - (totalsRowCount + SECTION_MARGIN_TOP + 1)
    const shownDetailFields = detailFields.slice(0, Math.max(0, detailRoom))
    const detailHeight = 1 + shownDetailFields.length + (framed ? 2 : 0) + SECTION_MARGIN_TOP
    ...
    const showTotals = take(totalsRowCount + SECTION_MARGIN_TOP)
    const showHint = take(1)
    // Before the model rows the table draws its separator, its header and its
    // rule, so only `left - 3` rows are actually available for models.
    const tableCapacity = left - 3
      ...
      if (view === undefined) {
        ...
        body.push(
          h(
            ...
            `${t(lang, 'historyTotalTokens')} ${formatTokens(view.totals.tokens)} (${t(lang, 'historyTotalsScope')}) · ${t(lang, 'historyTotalCost')} ${formatCny(view.totals.cost)}`,
          ),
        )
      }
    ...
    // ---- totals -------------------------------------------------------------
    // Every figure here is a CNY estimate from the same rate card, so the sum is
    // always summable; it spans the WHOLE scanned history rather than the weeks
    // on screen, and the label says so because the grid above does not.
    const totalsRows = []
    if (showTotals) {
      const costText = `${formatCny(view.totals.cost)}${view.totals.costIncomplete ? '*' : ''}`
      totalsRows.push(
        h(
          Box,
          ...
          h(
            ...
            h(
              Text,
              { wrap: 'truncate' },
              `${formatTokensFull(view.totals.tokens)} (${t(lang, 'historyTotalsScope')}) · ${t(lang, 'historyTotalCost')} ${costText} · ${t(lang, 'historyCacheHit')} ${formatPercent(view.totals.cacheHitRate)}`,
            ),
          ),
        ),
      )
          ...
          h(
            ...
            h(
              ...
              h(
                ...
                { wrap: 'truncate' },
                perProvider
                  .map(provider => `${provider.label} ${formatTokens(provider.tokens)} ${formatCny(provider.cost)}`)
                  .join(' · '),
              ),
            ),
          ),
