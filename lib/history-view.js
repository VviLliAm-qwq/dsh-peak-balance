  ...
  return function TokenHistoryScene({ React, ui, close }) {
        ...
        h(
          ...
          h(
            ...
            h(
              Text,
              { wrap: 'truncate' },
              `${formatTokensFull(view.totals.tokens)} · ${t(lang, 'historyTotalCost')} ${formatCny(view.totals.cost)}${view.totals.costIncomplete ? '*' : ''} · ${t(lang, 'historyCacheHit')} ${formatPercent(view.totals.cacheHitRate)}`,
            ),
          ),
        ),
      )
    ...
    if (showTable && tableRows >= 1) {
      ...
      for (const entry of view.models.slice(0, tableRows)) {
        ...
        tableNodes.push(tableRow(entry.model, [entry.model, formatTokens(entry.tokens), cost, formatPercent(entry.cacheHitRate), source], { dim: true }))
      }
    }
