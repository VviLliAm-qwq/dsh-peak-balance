 ...
 * wins; the host's persisted `/model` route is the next best answer, because a
 * conversation restored at boot has no tracker until its first request (see
 * `./model-pref.js`); and the historical default (`deepseek-official`) is the
 * last resort, so an untouched configuration behaves exactly as before.
 *
 * A preference is a guess about the NEXT request rather than a statement about
 * the conversation, so it carries its own `source` — callers treat it the way
 * they treat `focus` (a route with no quota interface stays quiet unless the
 * user pinned it by name).
 *
 * @param options.provider - provider of the focused conversation, if known.
 * @param options.preference - provider of the host's persisted `/model` route.
...
export function resolveTarget(options = {}) {
  ...
  const preferred = typeof options.preference === 'string' ? options.preference.trim() : ''
  if (preferred !== '') return { enabled: true, provider: preferred, source: 'preference' }
...
export async function collectQuota(options = {}) {
  ...
  // The opt-in covers both ways a read can leave documented ground: an adapter
  // that ships such an endpoint (see `needsUnofficial`) and a spec that admits
  // its endpoint is console-reverse-engineered — per provider, or for the whole
  // document. Without the setting the read is refused rather than silently made.
  const unofficial =
    needsUnofficial(adapter)
    || profile.stanza?.allowUnofficial === true
    || spec.allowUnofficial === true
  if (unofficial && config.allowUnofficialQuota !== true) {
    return failSnapshot('unsupported', {
      ...provenance,
      detail: `${adapter.ADAPTER_ID} needs allowUnofficialQuota (unofficial endpoint declared)`,
    })
  }
  if (
