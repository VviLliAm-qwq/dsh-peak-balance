 ...
 *   `{ ok: false, reason: 'network' | 'http' | 'invalid', status? }`.
 */
export async function fetchJson(url, options = {}) {
  ...
  try {
    ...
    if (response?.ok !== true) {
      // An unread failure body keeps its connection out of undici's pool until
      // the garbage collector reaches it; a 60 s poll loop would strand one
      // socket per failed request. Cancel it — without awaiting, so a stalled
      // body cannot hold this call open.
      try {
        const cancelled = response?.body?.cancel?.()
        if (cancelled !== undefined && typeof cancelled.catch === 'function') cancelled.catch(() => {})
      } catch {
        // Nothing to cancel: an already-consumed or synthetic body.
      }
    ...
    try {
      ...
      // A body that dies mid-read is the timeout/abort path: the transport
      // failed and the endpoint never answered. Only a body that arrives
      // intact and is not JSON is a payload failure.
      return { ok: false, reason: controller.signal.aborted ? 'network' : 'invalid', status }
    }
