...
export async function collect(options = {}) {
  ...
  // A transport failure has no status: name the reason instead of printing
  // `HTTP undefined` at the user (see `failureReasonOf`).
  const note = result => (result.status === undefined ? failureReasonOf(result) : `HTTP ${result.status}`)
  const failures = []
  if (creditsResult.ok !== true) failures.push(`credits: ${note(creditsResult)}`)
  if (keyResult.ok !== true) failures.push(`key: ${note(keyResult)}`)
