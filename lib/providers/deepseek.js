...
export async function collect(options = {}) {
  ...
  // `balance_infos: []` is a 200 that carries no balance: a successful snapshot
  // with zero meters renders as "no interface" (the account side falls back to
  // the unsupported state), which is a lie about a provider that answered. The
  // honest reading of "nothing usable in the payload" is `invalid`.
  if (parsed.balances.length === 0) return failSnapshot('invalid', provenance)
