...
export async function collect(options = {}) {
  ...
  results.forEach((result, index) => {
    ...
    if (result.ok !== true) {
      failures.push(`${withoutQuery(request.path)}: ${result.status === undefined ? result.reason : `HTTP ${result.status}`}`)
      return
    }
  ...
  const counterUnit = counterValue === undefined ? undefined : spec.spendUnit ?? unitOfPath(spec)
...
/**
 * A request path without its query string.
 *
 * These failure notes are rendered by `/quota`, and the host persists command
 * output in the session log — so a spec that carries a credential in the query
 * (`/api/usage?key=sk-…`) must not have it echoed back. The `?…` marker keeps
 * the note honest about the fact that a query was there.
 */
 ...
 * meter's unit is inherited. The lookup walks the spec's own meter declarations
 * — the evaluated meters carry `used`/`cap`/`remaining` only (see
 * `evaluateMeter`), while `usedPath` lives on the declaration.
 */
function unitOfPath(spec) {
