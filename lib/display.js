...
function meterLabel(lang, meter) {
  switch (meter.id) {
    ...
    // A relay's billing pair reports lifetime cumulative usage, not a period
    // figure, so it gets its own id rather than borrowing the period label.
    case 'lifetimeSpend': return t(lang, 'meterLifetimeSpend')
    case 'balance': return t(lang, 'meterBalance')
    // A vendor that splits its balance into a promotional part and a topped-up
    // part gets localized names for both instead of the adapters' English
    // fallback labels.
    case 'balance-granted': return t(lang, 'meterBalanceGranted')
    case 'balance-charged': return t(lang, 'meterBalanceCharged')
...
export function formatMeterAmount(lang, value, unit) {
  if (unit?.kind === 'money') return moneyText(value, unit)
...
function bareAmount(value, unit) {
  return unit?.kind === 'money' ? moneyText(value, unit) : formatDecimal(value)
}

/**
 * A money amount, carrying a symbol only when the provider named a currency.
 *
 * `formatMoney` defaults to CNY, which is right for the plugin's own cost
 * estimates and wrong for a provider's meter: a vendor that answers a bare
 * number (SiliconFlow's `totalBalance`) has not said the figure is yuan, and
 * stamping `¥` on it would be the very guess the adapters refuse to make.
 */
function moneyText(value, unit) {
  return typeof unit?.currency === 'string' && unit.currency !== ''
    ? formatMoney(value, unit.currency)
    : formatDecimal(value)
}
...
export function meterText(lang, meter) {
  ...
  if (cap !== undefined) {
    ...
    // A cap whose usage never arrived (a relay whose `/usage` read failed) still
    // shows the shape of the account — `余额 —/¥20.00` beats a bare label.
    const usedText = used === undefined ? '—' : bareAmount(used, unit)
    return `${label} ${usedText}/${bareAmount(cap, unit)}`
  }
 ...
 * @param options.quota - the account-side display model (see {@link quotaPart});
 *   `undefined` means this provider has nothing to show.
...
export function buildDisplay(options) {
  ...
  if (config.showBalance === true) {
    // The account part is the provider-neutral quota meter. An undefined
    // `quota` means "this provider has nothing to show" and is omitted rather
    // than papered over with an empty state.
    const account = quota === undefined ? undefined : quotaPart(lang, quota, atMs)
