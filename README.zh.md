...
**缓存**：插件激活时就读取增量缓存并直接画出方格图，所以打开场景是瞬时的；随后的扫描只刷新变化的部分。缓存按 `(路径, 大小, mtime)` 增量，内容已是最新时约 20~30 ms。缓存写在 `~/.dsh-tui/dsh-peak-balance-history.json`（可安全删除，删了会自动重建）；真正慢的只有本机第一次全量重建，而它会边扫边落盘，中途被打断也能续扫而不是重来。场景打开期间每 60 秒在数字背后静默刷新一次（状态行显示「刷新中」），关掉即停。
...
- **本机第一次打开 `/hist`（还没有缓存）需要几秒**：全量扫描会话日志（本机 341 份 / 80 MB 约 4~5 秒），期间显示扫描进度；扫描会边扫边落盘，中途被打断也能续扫。有缓存之后，打开场景直接由缓存出图。
